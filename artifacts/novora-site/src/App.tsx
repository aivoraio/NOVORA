...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUpRight, Bot, Check, Globe2, Layers3, Mail, Menu, Network, ScanLine, Sparkles, X } from "lucide-react";
...
function HeroField() {
  ...
  useEffect(() => {
    ...
    const points = Array.from({ length: reduced ? 24 : 52 }, (_, index) => ({
...
function Navbar() {
  ...
  const links = [["Vision", "#about"], ["Expertise", "#services"], ["Method", "#method"], ["Founder", "#founder"]];
  useEffect(() => {
  ...
  return <header className={`nav ${scrolled ? "scrolled" : ""}`}>
    <div className="container-n nav-inner">
      <a href="#top" className="brand" onClick={(event) => { event.preventDefault(); jump("#top"); }}>
        <img src={logo} className="brand-mark" alt="Novora monogram" /><span className="brand-word">NOVORA</span>
      </a>
      <nav className="nav-links">{links.map(([label, href]) => <a key={href} href={href}>{label}</a>)}</nav>
      <a className="nav-cta" href="#contact">Start a conversation <ArrowUpRight size={14} /></a>
      <button className="menu-toggle" type="button" aria-label={open ? "Close menu" : "Open menu"} onClick={() => setOpen(!open)}>{open ? <X size={22} /> : <Menu size={22} />}</button>
      {open && <nav className="mobile-menu">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)}>{label}</a>)}<a href="#contact" onClick={() => setOpen(false)}>Start a conversation</a></nav>}
    </div>
  </header>;
}
...
function Services() {
  ...
  return <div className="services-wrap">
    <div className="services-viewport" ref={viewportRef}><div className="services-container">
      {services.map((service, index) => { const Icon = service.icon; return <div className={`service-slide ${selected === index ? "is-active" : ""}`} key={service.no}>
    ...
    <main>
      <section className="hero"><HeroField /><div className="hero-grid" /><div className="container-n hero-layout">
        <div className="hero-copy">
          ...
          <Reveal className="delay-2"><p className="hero-sub">Novora is where intelligent technology meets a distinctly human point of view. We make ambitious ideas useful, beautiful, and real.</p><div className="hero-actions"><MagneticButton href="#services">Explore the studio <ArrowUpRight size={16} /></MagneticButton><a className="text-link" href="#about">Read our point of view <ArrowUpRight size={15} /></a></div><div className="hero-note"><ArrowDown size={14} /> Scroll to explore</div></Reveal>
        </div>
        <div className="hero-orbit" onPointerMove={portraitMove} onPointerLeave={portraitLeave}>
          ...
          <div className="portrait-shell"><img src={portrait} alt="Rayane Mazari, founder of Novora" /><div className="portrait-caption"><strong>Rayane Mazari</strong><span>Founder / Builder</span></div></div>
      ...
      <section className="section paper" id="about"><div className="container-n manifesto"><Reveal><div className="manifesto-index">01 — Point of view</div></Reveal><Reveal className="delay-1"><div className="manifesto-copy"><h2>Technology should feel <em>inevitable.</em></h2><p>Not louder. Not more complicated. The best tools disappear into the way people already think and work. We pair careful design with practical AI to create that feeling — a new capability that feels like it was always meant to be there.</p><div className="signature-line">Human curiosity, engineered clearly.</div></div></Reveal></div></section>
      ...
      <section className="section paper" id="method"><div className="container-n"><Reveal><div className="section-head"><div><span className="eyebrow">03 — How we work</span><h2>Clarity before <span className="serif-italic gold-text">complexity.</span></h2></div><p>Every Novora engagement has a beginning, a build, and a reason. No theatre. Just momentum you can feel.</p></div></Reveal><div className="method-grid"><Reveal><article className="method"><span className="method-number">01 / LISTEN</span><Sparkles className="method-icon" size={19} /><h3>Find the live wire.</h3><p>We start with the friction, the desire, and the odd little detail everyone else overlooks.</p></article></Reveal><Reveal className="delay-1"><article className="method"><span className="method-number">02 / FRAME</span><ScanLine className="method-icon" size={19} /><h3>Make the invisible clear.</h3><p>A focused direction replaces a sprawling brief. We define what matters before we make anything.</p></article></Reveal><Reveal className="delay-2"><article className="method"><span className="method-number">03 / BUILD</span><Bot className="method-icon" size={19} /><h3>Ship the first useful thing.</h3><p>Small, testable, alive. Our prototypes earn their next step by being useful on day one.</p></article></Reveal><Reveal className="delay-3"><article className="method"><span className="method-number">04 / TUNE</span><Check className="method-icon" size={19} /><h3>Leave it better.</h3><p>We refine the edges, document the thinking, and hand over systems that keep giving.</p></article></Reveal></div></div></section>
       <section className="section ivory" id="founder"><div className="container-n founder-layout"><Reveal><div className="founder-visual"><div className="orbit-ring" /><div className="portrait-shell"><img src={portrait} alt="Portrait of Rayane Mazari" /><div className="portrait-caption"><strong>Rayane Mazari</strong><span>Founder / Creative technologist</span></div></div></div></Reveal><Reveal className="delay-1"><div className="founder-copy"><span className="eyebrow">04 — Behind Novora</span><h2>Built by someone who still <em>gets curious.</em></h2><p>Rayane Mazari started Novora to close the distance between a good idea and its first real expression. He works across design, code, and emerging intelligence — close enough to the details to keep the original spark intact.</p><div className="founder-quote">“The future is not a place we arrive at. It is a material we learn to shape.”</div><a className="text-link" href="mailto:hello@novora.studio?subject=Hello%20Novora">Write to Rayane <ArrowUpRight size={15} /></a><div className="socials"><a className="social" href="https://www.linkedin.com" target="_blank" rel="noreferrer" aria-label="LinkedIn"><img src={linkedinAsset} alt="" /></a><a className="social" href="https://www.instagram.com" target="_blank" rel="noreferrer" aria-label="Instagram"><img src={instagramAsset} alt="" /></a><a className="social" href="https://github.com" target="_blank" rel="noreferrer" aria-label="GitHub"><img src={githubAsset} alt="" /></a></div></div></Reveal></div></section>
      <section className="contact" id="contact"><div className="container-n"><Reveal><span className="eyebrow">05 — A useful next step</span><h2>Have an idea<br />worth <em className="serif-italic">building?</em></h2><p>Tell us what you are making, what is getting in the way, or simply what you cannot stop thinking about.</p><div className="contact-actions"><MagneticButton href="mailto:hello@novora.studio?subject=Let%27s%20build%20something">Start a conversation <Mail size={16} /></MagneticButton><a className="text-link" href="#top">Back to the beginning <ArrowUpRight size={15} /></a></div></Reveal></div></section>
    </main>
    <footer className="footer"><div className="container-n"><div className="footer-top"><div className="footer-brand"><a href="#top" className="brand"><img src={logo} className="brand-mark" alt="Novora monogram" /><span className="brand-word">NOVORA</span></a><p className="footer-copy">Creative technology for people with somewhere interesting to go.</p></div><div className="footer-nav"><div><h4>Explore</h4><a href="#about">Point of view</a><a href="#services">The studio</a><a href="#method">How we work</a></div><div><h4>Connect</h4><a href="mailto:hello@novora.studio">hello@novora.studio</a><a href="#contact">Start a conversation</a><a href="#founder">Meet the founder</a></div></div></div><div className="footer-bottom"><span>© 2025 Novora Studio</span><span>Made for the next idea</span></div></div></footer>
  </div>;
}
